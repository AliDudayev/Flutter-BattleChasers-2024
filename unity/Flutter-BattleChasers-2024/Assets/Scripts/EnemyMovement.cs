...
public class EnemyMovement : MonoBehaviour
{
    ...
    private void MoveTowardsPlayer()
    {
        ...
        // Use Rigidbody to move the enemy to avoid overlap
        Rigidbody rb = GetComponent<Rigidbody>();
        if (rb != null)
        {
    ...
    private void MoveAwayFromPlayer()
    {
        ...
        if (rb != null)
        {
            rb.MovePosition(transform.position + direction * (moveSpeed / 2) * Time.deltaTime); // Half the move speed
        }
        animator.SetBool("Running", true); // Play running animation
        //------------------------------------------------------------------------------------------------
    }
    ...
    private void ShootProjectile()
    {
        ...
        // Instantiate the projectile at the spawn location
        GameObject projectile = Instantiate(projectilePrefab, projectileSpawnLocation.transform.position, Quaternion.identity);

        // Calculate the direction towards the player, ignoring the Y-axis
        Vector3 direction = new Vector3(playerTransform.position.x - projectileSpawnLocation.transform.position.x, 0, playerTransform.position.z - projectileSpawnLocation.transform.position.z).normalized;
