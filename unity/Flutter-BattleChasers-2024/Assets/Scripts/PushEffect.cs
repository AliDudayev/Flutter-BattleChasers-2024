...
public class PushEffect : MonoBehaviour
{
    ...
    private IEnumerator ChangeColorTemporarily(Color hitColor, float duration)
    {
        ...
        for (int i = 0; i < childRenderers.Count; i++)
        {
            ...
            {
                if (material.HasProperty("_Color") && material.GetColor("_Color") is Color)
                {
        ...
        while (elapsedTime < duration)
        {
            for (int i = 0; i < childRenderers.Count; i++)
            {
                ...
                foreach (Material material in childRenderers[i].materials)
                {
                    if (material.HasProperty("_Color") && material.GetColor("_Color") is Color)
                    {
                        material.SetColor("_Color", Color.Lerp(hitColor, originalColors[colorIndex], elapsedTime / duration));
                    }
                    colorIndex++;
                }
            }
        ...
        for (int i = 0; i < childRenderers.Count; i++)
        {
            foreach (Material material in childRenderers[i].materials)
            {
                if (material.HasProperty("_Color") && material.GetColor("_Color") is Color)
                {
                    material.SetColor("_Color", originalColors[finalColorIndex]);
                }
                finalColorIndex++;
            }
        }
